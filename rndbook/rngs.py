# -------------------------------------------------------------------------
# This is an ANSI C library for multi-stream rndbook number generation.
...
#  * where the statistical 'goodness' of the rndbook number generator is
#  * important.  The library supplies 256 streams of rndbook numbers; use
...
#  *
#  * The generator used in this library is a so-called 'Lehmer rndbook number
#  * generator' which returns a pseudo-rndbook number uniformly distributed
...
def random(): 
  #/* ---------------------------------------------------------------------
  #* Random is a Lehmer generator that returns a pseudo-rndbook real number
...
def plantSeeds(x): 
  # /* --------------------------------------------------------------------
  #  * Use this function to set the state of all the rndbook number generator
...
def putSeed(x):
  ...
  #  * the rndbook number generator according to the following conventions:
...
def getSeed():
  # /* --------------------------------------------------------------------
  #  * Use this (optional) procedure to get the current state of the rndbook
  #  * number generator.
  #  * --------------------------------------------------------------------
  #  */
  return seed[stream]
...
def selectStream(index):
  #/* ------------------------------------------------------------------
  #* Use this function to set the current rndbook number generator
  #* stream -- that stream from which the next rndbook number will come.
  #* ------------------------------------------------------------------
  #*/
  global stream 
